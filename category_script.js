...
const products = [
    ...
    { id: 3, player: "LeBron James", desc: "2003 Topps Chrome Refractor Rookie. Flawless chrome border finish.", price: "$35,000.00", grade: "BGS 9.5+", type: "ROOKIE", image: "images/lebron.png", series: "Topps Chrome", era: "Modern" },
...
function renderProducts() {
    ...
    let title = "All Trading Cards";

    if (categoryType === 'rookies') {
        filteredProducts = products.filter(p => p.type === 'ROOKIE' || p.type === 'PRIZM');
        title = "Rookie Cards";
    } else if (categoryType === 'autos') {
        filteredProducts = products.filter(p => p.type === 'AUTO/PATCH');
        title = "Autographs & Patches";
    } else if (categoryType === 'vintage') {
        filteredProducts = products.filter(p => p.type === 'ROOKIE' && p.player.includes('Jordan'));
        title = "Vintage Classics";
    } else if (categoryType === 'inserts') {
        filteredProducts = products.filter(p => p.type.includes('NUM') || p.type === '8-BIT');
        title = "Rare Inserts";
    }
    ...
    if (filteredProducts.length === 0) {
        grid.innerHTML = '<p style="color: var(--text-secondary);">No cards found in this category.</p>';
        return;
    }
